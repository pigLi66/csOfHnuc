...
import org.dahanjsy.autoredpack.service.model.Req
import org.dahanjsy.autoredpack.service.model.Resp
...
object ApiModelTransfer {
    ...
    fun <R> invokeService(req: Req<R>): R {
        // 1、组织接口数据
        ...
        val inBodyStr = execute(requestUrl, req.method(), outBodyStr) {
            if (req.authorize()) {  // 如果需要token
                it.setRequestProperty(ACCESS_TOKEN_HEADER_KEY, UserContext.childToken)
            }
        }
        ...
        if (rootNode.path(ERR_CODE_NODE_KEY).asInt(0) != 0) {
            throw RuntimeException(rootNode.path(ERR_MSG_NODE_KEY).asText())
        }

//        (types[0] as ParameterizedType).rawType
//        var result = (type as Class<*>)

        when (val type = (req.javaClass.genericSuperclass as ParameterizedType).actualTypeArguments[0]) {
            is Class<*> -> {
            ...
            else -> throw RuntimeException("解析请求参数泛型失败")
        }
    }
    ...
    private fun execute(
        ...
        try {
            ...
            if (requestMethod != "GET" && outputStr.isNotBlank() && outputStr != "{}") {
                connection.outputStream.write(outputStr.toByteArray())
            }
            ...
            val errorJsonStr = connection.errorStream.bufferedReader().use { it.readText() }  // defaults to UTF-8
