  ...
  methods: {
    searchEnginesChanged() {
      for (let x = 0; x < this.searchEngines.options.length; x++) {
        ...
        if (this.searchEngines.select == this.searchEngines.options[x].value) {
          ...
          this.searchEngines.main_url = this.searchEngines.options[x].mainUrl;
    ...
    async searchEnginesGet() {
      try {
        ...
        for (let s = 0; s < res.data.length; s++) {
          const suggestUrl = defaultSuggestUrl;
          const suggestFunc = defaultSuggestFunc;
        ...
        this.searchEngines.main_url = this.searchEngines.options[0].mainUrl;
        this.searchEngines.suggest_url = this.searchEngines.options[0].suggestUrl;
        this.searchEngines.suggest_func = this.searchEngines.options[0].suggestFunc;
    ...
    async autoComplete(queryString: any, cb: any = null) {
      if (
        ...
        const autoCompleteUrl = this.searchEngines.suggest_url.replace(
          "%word%",
          this.word
        );
        try {
